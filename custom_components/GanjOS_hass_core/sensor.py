...
async def create_sensor_entities(hass, context_id: str, config: dict):
    """
    Creates sensor entities for a given context (area or plant stage).
    ...
    _LOGGER.info(f"Created {len(entities)} sensor entities for {context_id}.")
...
class GanjosSensor(SensorEntity):
    def __init__(self, context_id, parameter, config, entity_id):
        ...
        self._config = config
        self._state = config.get("value", 0)
        self._attr_native_unit_of_measurement = config.get("unit", "")
        self._attr_icon = config.get("icon", "mdi:eye")
